"""
keyboard_control.py — v2_non_encoder_robot
==========================================

Keyboard control using observation biasing.
The SAC policy stays in full control of balance.
Keys bias the observation to make the policy
naturally drive in the desired direction.

HOW IT WORKS:
  Normal mode: policy reads [pitch, pitch_rate, yaw, yaw_rate]
               and decides motor commands to stay balanced.

  Key pressed:  we add a small bias to pitch or yaw observation.
               Policy thinks robot is slightly tilted/rotated.
               Policy naturally corrects — robot moves that direction.
               Balance is maintained throughout.

  Key released: bias removed immediately.
               Robot returns to stationary balance on its own.
...
Run: python keyboard_control.py
...
# Start conservative — increase if movement is too slow
...
def on_press(key):
    ...
    with bias_lock:
        try:
            ...
            # Special keys
            if key == keyboard.Key.space:   # stop
                bias["pitch"] = 0.0
                bias["yaw"]   = 0.0


def on_release(key):
    # Release key → clear that axis bias immediately
...
if not os.path.exists(norm_path):
    ...
    print("Solution: Let train_sac.py run past 10,000 steps first.")
    exit(1)
...
print("=" * 50)
print("  Keyboard Control — Observation Bias Mode")
print("=" * 50)
print("  i     = forward")
print("  k     = backward")
print("  j     = turn left")
...
try:
    while running:
        ...
        if pitch_bias != 0.0 or yaw_bias != 0.0:
            # Unscale: raw = normalised * std + mean
            raw = obs_biased[0] * np.sqrt(env.obs_rms.var + 1e-8) \
                  + env.obs_rms.mean

            # Bias pitch (obs[0]) and yaw (obs[2])
            raw[0] += pitch_bias
            raw[2] += yaw_bias

            # Clip to observation bounds before rescaling
            raw[0] = np.clip(raw[0], -np.pi, np.pi)
            raw[2] = np.clip(raw[2], -np.pi, np.pi)

            # Rescale: normalised = (raw - mean) / std
            obs_biased[0] = (raw - env.obs_rms.mean) \
